<script setup lang="ts">
interface Props {
  driverName: string,
  plateNumber: string
}
defineProps<Props>()
</script>


<template>
  <div class="flex flex-col w-6/12 max-md:ml-0 max-md:w-full">
    <div class="flex flex-col grow px-14 py-16 w-full bg-zinc-300 rounded-[40px] max-md:px-5 max-md:mt-10">
      <div class="flex shrink-0 self-center w-20 h-20 bg-white rounded-full"></div>
      <div class="flex shrink-0 bg-white rounded-2xl h-[100px] w-[130px]"></div>
    </div>
    <div
      class="px-1.5 pt-1.5 pb-6 mt-5 max-w-full text-4xl font-semibold text-black whitespace-nowrap rounded-2xl bg-slate-400 w-[800px] max-md:pr-5">
      {{ driverName }}
    </div>
    <div class="px-1.5 py-1.5 mt-7 text-4xl font-semibold text-black rounded-2xl bg-slate-400 max-md:pr-5">
      Plate number: {{ plateNumber }}
    </div>
  </div>
</template>